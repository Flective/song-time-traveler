...
# --- CONFIGURATION ---
st.set_page_config(page_title="Fiat Musica: Triangulation Platform v5.1", layout="wide", page_icon="🎹")
...
def load_data():
    if os.path.exists(CSV_FILE):
        ...
        return df
    return pd.DataFrame(columns=[
        "Title", "Era", "Disc", "Version", "Score", "Status", "Notes"
    ])
...
with st.sidebar:
    ...
    with st.form("quick_add_form", clear_on_submit=True):
        ...
        new_score = st.slider("Initial Score", 1.0, 10.0, 7.0, 0.5)
...
st.markdown("### *Experience Songs as Living Ideals* | **v5.1 Complete Edition**")
        ...
        if selected_song:
            ...
            with song_tab3:
                ...
                if song_reviews.empty:
                    ...
                    for _, review in song_reviews.iterrows():
                        with st.container(border=True):
                            col1, col2 = st.columns([3, 1])
                            with col1:
                                st.markdown(f"**{review['Song_Version']}** - {review['Review_Type']}")
                                st.caption(f"By {review['Reviewer']} on {review['Timestamp']}")
                            with col2:
...
with tab4:
    ...
    # Add new review (standalone form)
    with st.expander("➕ Add New Review", expanded=False):
...
with tab9:
    ...
    with settings_tab3:
        ...
        st.markdown(f"**Version:** 5.1 Complete Edition")
        ...
        with col2:
            if st.button("📊 Show File Stats"):
                ...
                files = [CSV_FILE, SONGS_MASTER_FILE, VOTING_FILE, PERSONAL_ALBUMS_FILE, REVIEWS_FILE]
                for f in files:
                    if os.path.exists(f):
...
st.caption(f"Fiat Musica v5.1 Complete Edition - The Triangulation Platform | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")