...
PAGE_CONFIG = {
    "Creative Studio": ["My Album Builder", "Virtual Songs", "Personas", "Living Archive"],
...
with st.sidebar:
    ...
    with col1:
        if st.button("FM", key="logo_home", help="Go to Home"):
            st.session_state.current_page = "My Album Builder"
            st.rerun()
    with col2:
        st.markdown("**Fiat Musica**")
        st.caption("v7.0")
    
    # Theme indicator
    current_theme_info = get_theme(st.session_state.current_theme)
    st.caption(f"Theme: {current_theme_info.get('name', 'Default')}")
    
    st.divider()
    
    # Hierarchical Navigation with working links
    for category, pages in PAGE_CONFIG.items():
        st.markdown(f'<p class="nav-category-label">{category}</p>', unsafe_allow_html=True)
    ...
    with st.expander("System Overview", expanded=False):
        ...
        total_manifestations = sum(
            len(s.get('manifestations', [])) 
        ...
        if virtual_songs_count > 0:
            ...
            st.metric("Triangulation", f"{density:.1f}x")
        
        active_personas = sum(1 for p in st.session_state.personas.values() if p.get('active', True))
        st.metric("Personas", active_personas)
        
        st.metric("Founding Members", f"{len(st.session_state.founding_members)}/100")
...
# Customize banner images per page - add images to a 'banners' folder
PAGE_BANNERS = {
    # "My Album Builder": "banners/album_builder.png",
    # Add paths to your banner images here
}

def render_page_banner(page_name):
    """Render configurable banner for each page"""
    banner_path = PAGE_BANNERS.get(page_name)
    
    if banner_path and os.path.exists(banner_path):
        st.image(banner_path, use_container_width=True)
    else:
        # Placeholder banner - shows path hint for adding your own
        st.markdown(f"""
        <div class="page-banner">
            <span style="color: var(--text-muted); font-size: 0.75rem;">
                Add banner: banners/{page_name.lower().replace(' ', '_')}.png
            </span>
        </div>
        """, unsafe_allow_html=True)

# --- MAIN INTERFACE ---
st.title(f"Fiat Musica: {st.session_state.current_page}")
render_page_banner(st.session_state.current_page)

# --- TAB 1: MY ALBUM BUILDER ---
if st.session_state.current_page == "My Album Builder":
    st.subheader("Build Your Personal Album")
...
if st.session_state.current_page == "Settings":
    ...
    st.markdown(f"""
    - **Version:** 7.0 - The Platonic Album OS
    - **Database:** JSON-based
